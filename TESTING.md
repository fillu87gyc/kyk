...
| ロジック一部のみテスト | ロジック層 **関数カバレッジ 100%** + E2E 27 シナリオ |
...
**16 本の Critical User Journey** に分解し、すべてに自動テストを割り当てた。
...
すべての CUJ に最低 1 本の自動テストが対応する。**未カバーの CUJ は無い（16/16 = 100%）。**
...
ノード層（game/player/bullet_manager/main）とロジック層の結線を通しで検証する。**計 27 本。**
...
- **関数カバレッジ: 69/69 = 100%**
- **単体テスト本数 / 関数数 = 159 / 69 ≒ 230%**（＝「100% を超える単体テスト」）
...
- ✅ 16 本の CUJ すべてに自動テストが対応し、E2E 27 シナリオで通し検証される
